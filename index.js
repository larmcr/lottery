...
files.forEach((file) => {
  if(file.includes('json')) {
    ...
    if (Array.isArray(json)) {
      arr = arr.concat(_.map(json, (item) => _.orderBy(item.numeros)));
    } else {
      arr.push(_.orderBy(json.numeros));
    }
  }
});
...
arr.forEach((item) => {
  const combs = _.map(combinations(item, LEN), _.uniq).filter((item) => item.length === LEN);
  combs.forEach((ele) => {
    const str = ele.join('-');
